...
Reporting on information from the _{product-title}_ monitoring system is important for strategical or operational decisions.
...
These reports can be executed on demand or on a pre-defined schedule within _{product-title}_.

NOTE: Originally _Database Reports_ were introduced to create reports working on data stored in the _{product-title}_ database only.
      This is no longer mandatory, also performance data can be used.
      Theoretically the reports do not necessarily need to be _{product-title}_ related.

WARNING: The _{product-title} Report Engine_ allows the creation of various kinds of reports and also supports distributed report repositories.
...
The _{product-title} Report Engine_ uses the _JasperReport_ library to create reports in various output formats.
Each report template must be a `*.jrxml` file.
The _{product-title} Report Engine_ passes a _JDBC_ Connection to the _{product-title} Database_ to each report on execution.
...
To add a new _JasperReport_ report to the _Local {product-title} Report Repository_, the following steps are required.
...
TIP: We recommend always to use the same _Jaspersoft Studio_ version as the _JasperReport_ library {product-title} uses.
     Currently {product-title} uses version {jasperReportsVersion}.

[[ga-database-reports-connect-database]]
==== Connect to the {product-title} Database

In order to actually create SQL statements against the _{product-title} database_ a `database Data Adapter` must be created.
...
NOTE:  When using the _Measurements Datasource_ within a report a `HTTP` connection to the _Measurements API_ is only established if the report is NOT running within {product-title}, e.g. when used with _Jaspersoft Studio_.
...
There are a couple of helper methods to help creating reports in _{product-title}_.
...
To establish a secure connection to the _Measurements API_ the public certificate of the running _{product-title}_ must be imported to the _Java Trust Store_.
In Addition _{product-title}_ must be configured to use that _Java Trust Store_.
 ...
 * Only a _JDBC Datasource_ to the _{product-title} Database connection_ can be passed to a report, or no datasource at all.
