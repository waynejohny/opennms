...
All default reports of _{product-title}_ are located in `$OPENNMS_HOME/etc/report-templates`.
Each `.jrxml` file located there can be modified and the changes are applied the next time a report is created by _{product-title}_.

When a subreport has been modified _{product-title}_ will detect a change based on the report's `lastModified` time and will recompile the report.
...
TIP: If unsure, simply delete all `.jasper` files and _{product-title}_ will automatically compile the subreports if needed.
