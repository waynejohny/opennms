...
This monitor has two parts, the first one is the monitor class _DhcpMonitor_ executed by _Pollerd_ and the second part is a background daemon _Dhcpd_ running inside the {product-title} JVM and listening for DHCP responses.
...
The _Dhcpd_ background daemon is disabled by default and has to be activated in `service-configuration.xml` in {product-title} by setting `service enabled="true"`.
...
If you try to start {product-title} without the _{opennms-package-base-name}-plugin-protocol-dhcp_ you will see the following error message in `output.log`:
...
CAUTION: Make sure no DHCP client is running on the {product-title} server and using port UDP/68.
...
| `macAddress`       | The MAC address which {product-title} uses for a dhcp request                             | required           | `00:06:0D:BE:9C:B2`
| `myIpAddress`      | This parameter will usually be set to the IP address of the {product-title} server,
                       ...
                       same subnet as the {product-title} server, and without the aid of an external relay.
...
image::02_01_dhcp-monitor-messages-broadcast.png[]

.Visualization of DHCP message flow in relay mode
image::02_02_dhcp-monitor-messages-unicast.png[]
...
.Step 2: Enable the {product-title} _Dhcpd_ daemon in `service-configuration.xml`
[source, xml]
----
...
.Step 3: Configure _Dhcpd_ to test a DHCP server in the same subnet as the {product-title} server.
[source, xml]
----
<DhcpdConfiguration
       port="5818"
