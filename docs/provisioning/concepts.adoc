...
Provisiond receives "requests" for adding managed entities via 2 basic mechanisms, the {product-title} traditional "New Suspect" event, typically via the _Discovery daemon_, and the import requisition (XML definition of node entities) typically via the Provisioning Groups UI.
