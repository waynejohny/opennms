...
{product-title} currently provides support for detecting, polling and collecting metrics from WS-Man endpoints.

[[ga-performance-mgmt-collectors-wsman-setup]]
===== Setup

Before setting up {product-title} to communicate with a WS-Management agent, you should confirm that it is properly configured and reachable from the {product-title} system.
...
Once validated, add the agent specific details to the {product-title} configuration, defined in the next section.
