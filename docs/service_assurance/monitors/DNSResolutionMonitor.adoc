...
The DNS resolution monitor, tests if the node label of an {product-title} node can be resolved.
This monitor uses the name resolver configuration from the poller configuration or from the operating system where {product-title} is running on.
...
The response from the A record lookup can be any address, it is not verified with the IP address on the {product-title} IP interface where the monitor is assigned to.
...
| `nameserver`      | The DNS server to query for the records. | optional | Use the servers defined by the system running {product-title}
...
In {product-title} the DNS server is the node and the DnsMonitor will send a lookup request for a given A record to the DNS server IP address.
