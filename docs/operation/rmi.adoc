...
By default, the RMI port in the _{product-title}_ server is disabled, for security reasons.  If you wish to enable it so you can access _{product-title}_ through jconsole, remote-manage _{product-title}_, or use the remote poller over RMI, you will have to add some settings to the default _{product-title}_ install.

== Enabling RMI

To enable the RMI port in _{product-title}_, you will have to add the following to the `${OPENNMS_HOME}/etc/opennms.conf` file.  If you do not have an `opennms.conf` file, you can create it.
