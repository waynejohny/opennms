...
Syslog messages sent over the network to _{product-title}_ can be transformed into events according to pre-configured rules.

IMPORTANT: The `Syslogd` service daemon, which enables _{product-title}_ to receive syslog messages over the network, must be enabled for this functionality to work. This service daemon is *disabled* by default.

=== Parsers

Different parsers can be used to convert the syslog message fields into _{product-title}_ event fields.
...
If you wish to customize the set of patterns, you can put a new set of patterns into a `syslog-grok-patterns.txt` in the `etc` directory for _{product-title}_.
...
The patterns should be arranged in the file from most specific to least specific since the first pattern to successfully match the syslog message will be used to construct the _{product-title}_ event.
